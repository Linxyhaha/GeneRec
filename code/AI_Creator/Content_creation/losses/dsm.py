...
def anneal_dsm_score_estimation(scorenet, x, labels=None, loss_type='a', hook=None, cond=None, cond_mask=None, gamma=False, L1=False, all_frames=False):
    ...
    if version == "SMLD":
        ...
        alphas = net.alphas
        if labels is None:
            labels = torch.randint(0, len(alphas), (x.shape[0],), device=x.device) # batch size, 随机给它们抽一个t，也就是alpha
        used_alphas = alphas[labels].reshape(x.shape[0], *([1] * len(x.shape[1:]))) # 随机选中的batchsize个时刻的alpha，reshape成(bs,1,1,1)
        if gamma:
