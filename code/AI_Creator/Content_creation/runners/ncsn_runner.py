...
def conditioning_fn(config, X, num_frames_pred=0, prob_mask_cond=0.0, prob_mask_future=0.0, conditional=True):
    ...
    pred_frames = X[:, cond:cond+pred].reshape(len(X), -1, imsize, imsize) # 合并通道, channel * frames (bs,frame*channel, img_size,img_size)

    # Condition (Past)
    cond_frames = X[:, :cond].reshape(len(X), -1, imsize, imsize) # (bs,4*3,64,64)
...
class NCSNRunner():
    ...
    def train(self):
        ...
        if self.config.data.dataset.upper() == 'FFHQ':
            ...
            dataset, test_dataset = get_dataset(self.args.data_path, self.config, video_frames_pred=self.config.data.num_frames, start_at=self.args.start_at, interaction_path=self.args.interaction_path)
        ...
        if self.args.resume_training:
            ...
            states = torch.load(self.args.pretrained_path)
        ...
        # Conditional
        conditional = self.config.data.num_frames_cond > 0 # True
        ...
        init_samples_shape = (n_init_samples, self.config.data.channels*self.config.data.num_frames, self.config.data.image_size, self.config.data.image_size) # (n,c*f,w,h)
        if self.version == "SMLD":
            ...
            if getattr(self.config.model, 'gamma', False):
                ...
                init_samples = torch.randn(init_samples_shape, device=self.config.device) # random 采样
        ...
        for epoch in range(start_epoch, self.config.training.n_epochs):
            for batch, (X, y) in enumerate(dataloader):
                ...
                X = data_transform(self.config, X) # rescale, normalization, etc.
                X, cond, cond_mask = conditioning_fn(self.config, X, num_frames_pred=self.config.data.num_frames,
                                                     ...
                                                     conditional=conditional) # X是pred_frames (train_frames), cond 是 masked conditional frames
                ...
                if (step % self.config.training.snapshot_freq == 0 or step % self.config.training.sample_freq == 0) and self.config.training.snapshot_sampling:
                    ...
                    if conditional:
                        try:
                            ...
                            test_iter = iter(test_loader)
                            test_X, test_y = next(test_iter)
                        test_X = test_X[:len(init_samples)].to(self.config.device) # 取training batch个，最多36个
                        test_X = data_transform(self.config, test_X) # normalization,rescale之类的
                        test_X, test_cond, test_cond_mask = conditioning_fn(self.config, test_X, num_frames_pred=self.config.data.num_frames,
                                                                            prob_mask_cond=getattr(self.config.data, 'prob_mask_cond', 0.0),
                                                                            prob_mask_future=getattr(self.config.data, 'prob_mask_future', 0.0),
                                                                            conditional=conditional)
                    ...
                    if conditional:
                        ...
                        reali = inverse_data_transform(self.config, test_X.to('cpu')) #(bs,4*3,64,64)
                        condi = inverse_data_transform(self.config, test_cond.to('cpu')) #(bs,4*3,64,64)
                        if future > 0:
    ...
    def sample(self):
        ...
        if self.config.data.dataset.upper() == 'FFHQ':
            ...
            dataset_train, dataset_test = get_dataset(self.args.data_path, self.config, video_frames_pred=self.config.data.num_frames,interaction_path=self.args.interaction_path)
    ...
    def video_gen(self, scorenet=None, ckpt=None, train=False):
        ...
        calc_ssim = getattr(self.config.sampling, "ssim", False) #true
        calc_fvd = getattr(self.config.sampling, "fvd", False) #true
        # FVD
        if calc_fvd:
            ...
            if self.condp == 0.0 and self.futrf == 0:                           # (1) Prediction
                ...
                calc_fvd3 = False
            elif self.condp > 0.0 and self.futrf == 0:                         # (1) Pred + (3) Gen 我们要用这个
                calc_fvd1 = calc_fvd3 = self.condf + self.config.sampling.num_frames_pred >= 4 #10
                calc_fvd2 = False
            elif self.condp > 0.0 and self.futrf > 0 and self.futrp > 0.0 and not self.prob_mask_sync:      # (1) Interp + (2) Pred + (3) Gen
                calc_fvd1 = self.condf + self.config.data.num_frames + self.futrf >= 10
        ...
        if scorenet is None:
            if self.args.pretrained_path is not None:
                logging.info(f"Loading ckpt {self.args.pretrained_path}")
                states = torch.load(self.args.pretrained_path, map_location=self.config.device)
                ckpt=self.config.sampling.ckpt_id
            else:
                if self.config.sampling.ckpt_id is None:
                    ckpt = "latest"
        ...
        def my_collate(batch):
            data, uids = zip(*batch)
            data = torch.stack(data).repeat_interleave(preds_per_test, dim=0)
            uids = torch.stack(uids).repeat_interleave(preds_per_test, dim=0)
            return data, uids
        ...
        elif self.condp > 0.0 and self.futrf == 0:                         # (1) Pred + (3) Gen 我们用的是这个，cfree
            num_frames_pred = self.config.sampling.num_frames_pred
        ...
        dataset_train, dataset_test = get_dataset(self.args.data_path, self.config, video_frames_pred=num_frames_pred, start_at=self.args.start_at, interaction_path=self.args.interaction_path)
        dataset = dataset_train if getattr(self.config.sampling, "train", False) else dataset_test
        ...
        if self.config.sampling.data_init:
            dataloader2 = DataLoader(dataset, batch_size=self.config.sampling.batch_size//preds_per_test, shuffle=False,
                                     num_workers=self.config.data.num_workers, drop_last=False, collate_fn=my_collate)
        ...
        self.u_ids = None
        # for i, (real_, u_ids) in tqdm(enumerate(dataloader2), total=min(max_data_iter, len(dataloader2)), desc="\nvideo_gen dataloader"):

        for i, (real_, u_ids) in tqdm(enumerate(dataloader), total=min(max_data_iter, len(dataloader)), desc="\nvideo_gen dataloader"):
            self.u_ids = u_ids
            # only consider users in group for analysis
            if dataset.u2g.get(int(u_ids[0].item()),-1) == -1:
                continue
            self.g_idx = dataset.u2g[int(u_ids[0].item())]
            ...
            # Video Prediction
            if future == 0:
                num_frames_pred = self.config.sampling.num_frames_pred # nfp=4,12
            ...
            real, cond, cond_mask = conditioning_fn(self.config, real_, num_frames_pred=num_frames_pred,
                                                    prob_mask_cond=0.0, prob_mask_future=0.0, conditional=conditional) # real是dataset里面的pred帧，cond是前面的，但是因为random了所以可能没有condition上（这种情况就是generation)
            ...
            if self.config.sampling.data_init:
                ...
                real_init, _, _ = conditioning_fn(self.config, real_init, self.config.sampling.num_frames_pred, conditional=conditional)
                real_init = real_init.to(self.config.device) # (bs,12*3,64,64)
                real_init1 = real_init[:, :self.config.data.channels*self.config.data.num_frames]  # (bs,4*3,64,64)
                if self.version == "SMLD":
                    z = torch.randn_like(real_init1)
                    init_samples = real_init1 + float(self.config.model.sigma_begin) * z
                elif self.version == "DDPM" or self.version == "DDIM" or self.version == "FPNDM":
                    alpha = net.alphas[self.config.edit.T-1] #net.alphas[self.config.edit.T-1]
                    z = z / (1 - used_alphas).sqrt() if getattr(self.config.model, 'gamma', False) else z
            ...
            for i_frame in tqdm(range(n_iter_frames), desc="Generating video frames"):
                ...
                gen_samples = sampler(init_samples if i_frame==0 or getattr(self.config.sampling, 'init_prev_t', -1) <= 0 else gen_samples,
                                      ...
                                      gamma=getattr(self.config.model, 'gamma', False), denoise_T=getattr(self.config.edit, "denoise_T", False))
                ...
                if self.config.sampling.data_init:
                    if getattr(self.config.sampling, 'one_frame_at_a_time', False):
                        ...
                        real_init1 = real_init[:,(i_frame+1)*self.config.data.channels*self.config.data.num_frames:(i_frame+2)*self.config.data.channels*self.config.data.num_frames] # (bs,4*3,64,64)
                    if self.version == "SMLD":
                        z = torch.randn_like(real_init1)
                        init_samples = real_init1 + float(self.config.model.sigma_begin) * z
                    elif self.version == "DDPM" or self.version == "DDIM" or self.version == "FPNDM":
                        alpha = net.alphas[self.config.edit.T-1]
                        z = z / (1 - used_alphas).sqrt() if getattr(self.config.model, 'gamma', False) else z
                        init_samples = alpha.sqrt() * real_init1 + (1 - alpha).sqrt() * z
                else:
                    init_samples = z
            ...
            if future > 0 and prob_mask_future > 0.0 and not self.prob_mask_sync:
                ...
                if self.config.sampling.data_init:
                    try:
                        real_init, _ = next(data_iter2) # 拿到dataset里完整的cond+pred
                    except StopIteration:
                        if self.config.data.dataset.upper() == 'FFHQ':
                            dataloader2 = FFHQ_TFRecordsDataLoader([self.args.data_path], self.config.sampling.batch_size, self.config.data.image_size)
                        data_iter2 = iter(dataloader2)
                        real_init, _ = next(data_iter2)
                    real_init = data_transform(self.config, real_init) # 过一个normalization或rescale
                    real_init, _, _ = conditioning_fn(self.config, real_init, self.config.sampling.num_frames_pred, conditional=conditional) # real_init是pred部分的gt，也就是edit item原来的样子
                ...
                for i_frame in tqdm(range(n_iter_frames), desc="Generating video frames"):
                    ...
                    gen_samples = sampler(init_samples if i_frame==0 or getattr(self.config.sampling, 'init_prev_t', -1) <= 0 else gen_samples,
                                          ...
                                          clip_before=getattr(self.config.sampling, 'clip_before', True),
                                          t_min=getattr(self.config.sampling, 'init_prev_t', -1), log=True if not train else False,
                                          gamma=getattr(self.config.model, 'gamma', False), edit_T=getattr(self.config.edit, "T", None))
                    gen_samples = gen_samples[-1].reshape(gen_samples[-1].shape[0], self.config.data.channels*self.config.data.num_frames,
                                                          self.config.data.image_size, self.config.data.image_size)
                    pred_samples.append(gen_samples.to('cpu'))
                ...
                if calc_fvd3:
                    ...
                    if self.config.sampling.data_init:
                        try:
                            ...
                            data_iter2 = iter(dataloader2)
                            real_init, _ = next(data_iter2)
                        real_init = data_transform(self.config, real_init) # (bs, frames_cond+frames_pred, channels, imsz, imsz)
                        real_init, _, _ = conditioning_fn(self.config, real_init, self.config.sampling.num_frames_pred, conditional=False) #(bs,pred_frames*3,64,64)
                        real_init = real_init.to(self.config.device) 
                        real_init1 = real_init[:, :self.config.data.channels*self.config.data.num_frames]
                        if self.version == "SMLD":
                            z = torch.randn_like(real_init1)
            ...
            if i == 0 or preds_per_test == 1 or 1: # Save first mini-batch or save them all
                cond = cond_original
                ...
                def save_pred(pred, real,u_id):
                    ...
                    padding_hor = 0.5*torch.ones(*pred_im.shape[:-1], 2)
                    # real_data = torch.cat([padding_hor,pred_im], dim=-1)
                    pred_data = torch.cat([padding_hor, pred_im], dim=-1)
                    padding_ver = 0.5*torch.ones(*pred_im.shape[:-2], 2, pred_data.shape[-1])
                    data = torch.cat([padding_ver,pred_data], dim=-2)
                    # Save
                    nrow = ceil(np.sqrt((self.config.sampling.num_frames_pred)*pred.shape[0])/(self.config.sampling.num_frames_pred))
                    image_grid = make_grid(data, nrow=nrow, padding=6, pad_value=0.5)
                    if train:
                        save_image(image_grid, os.path.join(self.args.log_sample_path, f"videos_stretch_pred_{ckpt}_{i}.png"))
                    else:
                        save_image(image_grid, os.path.join(self.args.video_folder, f"G{self.g_idx}_U{int(u_id)}_({i%2})_videos_stretch_pred.png"))
                        # save_image(image_grid, os.path.join(self.args.video_folder, f"U{int(u_ids[0].item())}_({i%2})_videos_stretch_pred_{ckpt}_{i}.png"))
                ...
                def save_interp(pred, real):
                    ...
                    if train:
                        ...
                        save_image(image_grid, os.path.join(self.args.video_folder, f"G{self.g_idx}_U{int(u_id)}_({i%2})_videos_stretch_interp.png"))
                ...
                def save_gen(pred):
                    ...
                    if train:
                        ...
                        save_image(image_grid, os.path.join(self.args.video_folder, f"G{self.g_idx}_U{int(self.u_ids[0].item())}_({i%2})_videos_stretch_interp.png"))
                ...
                if self.condp == 0.0 and self.futrf == 0:                           # (1) Prediction
                    ...
                    assert len(set(self.u_ids.tolist()))==1, print(self.u_ids)
                    save_pred(pred, real,self.u_ids[0].item())
    ...
    def test(self):
        ...
        if self.config.data.dataset.upper() == 'FFHQ':
            ...
            dataset, test_dataset = get_dataset(self.args.data_path, self.config,interaction_path=self.args.interaction_path)
    ...
    def fast_fid(self):
        ...
        if self.config.data.dataset.upper() == 'FFHQ':
            ...
            dataset, _ = get_dataset(self.args.data_path, self.config,interaction_path=self.args.interaction_path)
            dataloader = DataLoader(dataset, batch_size=self.config.sampling.batch_size, shuffle=True,
                                    num_workers=self.config.data.num_workers)
        data_iter = iter(dataloader)
        ...
        if conditional:
            if self.config.data.dataset.upper() == 'FFHQ':
                ...
                dataset_cond, _ = get_dataset(self.args.data_path, self.config,interaction_path=self.args.interaction_path)
            ...
            # Generate samples
                ...
                for i in tqdm(range(num_iters), desc="samples"):
                    ...
                    all_samples = sampler(init_samples, scorenet, cond=cond, cond_mask=cond_mask, final_only=True,
                                          ...
                                          gamma=getattr(self.config.model, 'gamma', False), denoise_T=getattr(self.config.edit, "denoise_T", False)).to('cpu')
        ...
        for ckpt in tqdm(range(self.config.fast_fid.begin_ckpt, self.config.fast_fid.end_ckpt + 1, self.config.fast_fid.freq),
            ...
            for i in range(num_iters):
                ...
                all_samples = sampler(init_samples, scorenet, cond=cond,
                                      ...
                                      subsample_steps=getattr(self.config.sampling, 'subsample', None),
                                      clip_before=getattr(self.config.sampling, 'clip_before', True),
                                      log=True, gamma=getattr(self.config.model, 'gamma', False),denoise_T=getattr(self.config.edit, "denoise_T", False)).to('cpu')
