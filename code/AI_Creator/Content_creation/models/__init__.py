...
def ddpm_sampler(x_mod, scorenet, cond=None, just_beta=False, final_only=False, denoise=True, subsample_steps=None,
                 ...
                 t_min=-1, gamma=False, denoise_T=False, **kwargs):
    ...
    if not denoise_T:
        ...
        for i, step in enumerate(steps):
            ...
            if clip_before:
                x0 = x0.clip_(-1, 1)
            x_mod = (c_alpha_prev.sqrt() * c_beta / (1 - c_alpha)) * x0 + ((1 - c_beta).sqrt() * (1 - c_alpha_prev) / (1 - c_alpha)) * x_mod # eqn. (3)
    ...
    else: # item edit from edit_T not random noise
        steps = range(0,denoise_T)
        if subsample_steps is not None:
            alphas = alphas[:denoise_T]
            if subsample_steps < len(alphas):
                skip = len(alphas) // subsample_steps
                steps = range(0, len(alphas), skip)
                steps = torch.tensor(steps, device=alphas.device)
        ...
        for i, step in enumerate(steps):
            ...
            c_beta, c_alpha, c_alpha_prev = betas[i], alphas[i], alphas_prev[i]
            labels = (step * torch.ones(x_mod.shape[0], device=x_mod.device)).long() # t -> t_emb 作为input
            grad = scorenet(x_mod, labels)
