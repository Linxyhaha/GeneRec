...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="outputs/cover-generation-samples"
    )
    ...
    parser.add_argument(
        ...
        default='hisrandom',
        type=str,
        help='retriving mode for user'
    )
    parser.add_argument(
        "--save_img",
        action="store_true",
        help='whether or not save grid image'
    )
    opt = parser.parse_args()
    # REC
    N_USER, N_ITEM = 7895, 4570
    REC_ITEM = 3933
    train_data = np.load('path_of_training_dict.npy',allow_pickle=True).item()
    cover_feat = torch.load('path_of_video_feature_RDM.pt')
    # group_user = np.load('/user_group/group_dict.npy',allow_pickle=True).item()
    # tgt_users = list(group_user.values())[0]+list(group_user.values())[1]
    tgt_users = random.sample(range(N_USER),500)
    print(f"test_users:{tgt_users}")
    ...
    clip_text_encoder = FrozenCLIPTextEmbedder(opt.clip_type).cuda()
    ...
    with torch.no_grad():
        with model.ema_scope():
            for n in trange(opt.n_iter, desc="Sampling"):
                all_samples = list()
                for prompts in tqdm(tgt_users, desc="data"):
                    print("sampling prompts:", prompts)
                    u_his = train_data[prompts]
                    if len(train_data[prompts])==0:
                        continue
                    REC_ITEM = 3933 # random.choice(range(N_ITEM))
                    c = cover_feat[torch.LongTensor([REC_ITEM]),torch.LongTensor(random.sample(range(30),8)),:].float().cuda()
                    c = c/c.norm(dim=1,keepdim=True)
                    if opt.mode=='hisall':
                        for idxx, each_i in enumerate(u_his):
                            c_ = cover_feat[torch.LongTensor([each_i]),torch.LongTensor([random.choice(range(30))]),:] if idxx==0 else torch.cat([c_,cover_feat[torch.LongTensor([each_i]),torch.LongTensor([random.choice(range(30))])]],dim=0)
                    elif opt.mode=='hisavg':
                        c_ = torch.mean(cover_feat[torch.LongTensor(u_his)],dim=1)
                    elif opt.mode=='hisrandom':
                        c_ = cover_feat[torch.LongTensor([random.choice(u_his)]),random.choice(range(30))]
                    c_ = (c_/c_.norm(dim=1,keepdim=True)).float().cuda()
                    ...
                    for x_sample in x_samples_ddim:
                        ...
                        Image.fromarray(x_sample.astype(np.uint8)).save(
                            os.path.join(sample_path, f"new-{base_count:05}.png"))
                        base_count += 1
                ...
                if not opt.skip_grid:
                    ...
                    # save torch file
                    # torch.save(grid,os.path.join(outpath,f'grid-{grid_count:04}-{opt.mode}-{opt.scale}s-csim-{n}.pt'))
                    if opt.save_img:
                        ...
                        Image.fromarray(grid.astype(np.uint8)).save(os.path.join(outpath, f'grid-{grid_count:04}-{opt.mode}-{opt.scale}s-csim-{n}.png'))
                        grid_count += 1
